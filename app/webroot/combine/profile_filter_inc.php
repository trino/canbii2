<SPAN debugtitle="combine/profile_filter_inc.php">
<?php
errorlog("include combine/profile_filter_inc.php");
...
<?php if($this->params['action']!='dashboard' && !$this->Session->read('User')){ ?>
    <a href="<?= $this->webroot;?>users/register" style="position: absolute;top:0;left:0;width:100%;height:100%;background:#82BECE;opacity:0.8;">
        <img src="<?= $this->webroot;?>images/trans.png" style="width:100%" />
    </a>
<?php } ?>
<ul class="column_left">
   <?php if($this->params['action']!='dashboard'){ echo '<div class="bg">'; } ?>
   ...
   <?php if($this->params['action']!='dashboard'){echo'</div><div class="bg">';}?>
   <label>Country</label>
   <select name="country" style="width: 100%;">
		<option style="padding-top: 20px; padding-bottom:20px" value="">Select Country</option>
        <?php
            foreach($countries as $cou){
                echo '<option value="' . $cou['Country']['countryName'] . '" ';
                if($country==$cou['Country']['countryName']){echo "selected='selected'";}
                echo '>' . $cou['Country']['countryName'] . '</option>';
            }
        ?>
   </select><br />
        ...
        <select name="age_group_to" style="width: 103px!important;margin-left:10px;">
    		...
    		<?php
                for($i=30;$i<=100;$i=$i+10) {
                    echo '<option value="' . $i . '"';
                    if($age_group_to==$i){echo ' selected="selected"';}
                    echo '>' . $i . '</option>';
                }
           ?>
        </select>
...
<?php }?>
        ...
        <select name="weight_from" style="width: 103px!important;float:left;">
    		...
    		<?php
                for($i=101;$i<=290;$i=$i+10)  {
                    echo '<option value="' . $i . '"';
                    if($weight_from==$i){echo ' selected="selected"';}
                    echo '>' . $i . '</option>';
                }
            ?>
        </select>
        <select name="weight_to" style="width: 103px!important;margin-left:10px;">
    		...
    		<?php
                for($i=110;$i<=290;$i=$i+10){
                    echo '<option value="' . $i . '"';
                    if($weight_to==$i){ echo ' selected="selected"';}
                    echo '>' . $i . '</option>';
                }
            ?>
        </select></span>
   ...
   <?php } else {
        if($this->params['action']!='dashboard'){ echo '<div class="bg">'; }?>
        <label style="display: block!important;">Years of Experience</label>
        <select name="years_of_experience_from" style="width: 103px!important;float:left;">
    		...
    		<?php
    		    for($i=1;$i<=50;$i++) {
                    echo '<option ';
                    if($_GET['years_of_experience_from']==$i||$i == $exp-2||($i == 1 && $exp-2<1)){ echo ' selected="selected"';}
        ...
        <select name="years_of_experience_to" style="width: 103px!important;margin-left:10px;">
    		...
    		<?php
                for($i=2;$i<=50;$i++){
                    echo '<option ';
                    if($_GET['years_of_experience_to']==$i||$i == $exp+2||($i ==50 && $exp+2>50)){ echo ' selected="selected"';}
                    echo ' value="' . $i . '">' . $i . '</option>';
                }
            ?>
        </select>
   ...
   <br />
   <?php if($this->params['action']!='dashboard'){ echo '</div><div class="clearfix"></div>';}?>

   
</ul>
<div class="clearfix"></div>
<?php if($this->params['controller']=='strains' && $this->params['action']=='index') { ?>
    <br />
    <br />
    <a class="blue more" href="javascript:void(0)" onclick="filternow();" style="margin-right: 15px;margin-top:20px;" id="filternow">Filter Now</a><a style="margin-top: 20px;" class="blue more" href="<?= $this->webroot;?>strains/<?= $strain['Strain']['slug'];?>" onclick="" id="">Clear Filter</a>
<?php } ?>
</div>
<?php if($this->params['action']!='dashboard') { ?>
    <style>
...
?>
    <script>
        function filternow(){
            ...
            var i=0;
            window.location = '<?= $this->webroot?>strains/<?= $strain['Strain']['slug']?>/?'+profile;
        }
    </script>
<?php
}
errorlog("include profile_filter_inc.php success");
?>
</SPAN>